...
, m_CreatedEnsembleAttributeMatrixPath()
...
//  parameters.push_back(SIMPL_NEW_INTEGER_FP("Phase Index", PhaseIndex, FilterParameter::Parameter, GeneratePrimaryStatsData));
  {
  ...
  // Table 3 - Dynamic rows and fixed columns, default data passed in
  {
    ...
    parameters.push_back(SIMPL_NEW_DYN_TABLE_FP("ODF", OdfData, FilterParameter::Parameter, GeneratePrimaryStatsData, true, false, 0, 5, 0));
  }
  {
    QStringList cHeaders;
    ...
    parameters.push_back(SIMPL_NEW_DYN_TABLE_FP("MDF", MdfData, FilterParameter::Parameter, GeneratePrimaryStatsData, true, false, 0, 5, 0));
  }
  {
    QStringList cHeaders;
    cHeaders << "Euler 1" << "Euler 2" << "Euler 3" << "Weight" << "Sigma";
    ...
    parameters.push_back(SIMPL_NEW_DYN_TABLE_FP("Axis ODF", AxisOdfData, FilterParameter::Parameter, GeneratePrimaryStatsData, true, false, 0, 5, 0));
  }
  //---------------------------
...
//---------------------------
  ...
  QStringList linkedProps("CreatedEnsembleAttributeMatrixPath");
  parameters.push_back(SIMPL_NEW_LINKED_BOOL_FP("Create Ensemble AttributeMatrix", CreateEnsembleAttributeMatrix, FilterParameter::Parameter, GeneratePrimaryStatsData, linkedProps));

  AttributeMatrixCreationFilterParameter::RequirementType amReq;
  parameters.push_back(SIMPL_NEW_AM_CREATION_FP("Created Ensemble AttributeMatrix", CreatedEnsembleAttributeMatrixPath, FilterParameter::Parameter, GeneratePrimaryStatsData, amReq));
...
void GeneratePrimaryStatsData::dataCheck()
{
  ...
  if(m_CreateEnsembleAttributeMatrix && !m_AppendToExistingAttributeMatrix)
  {
    ...
    DataContainer::Pointer dc = dca->getDataContainer(m_CreatedEnsembleAttributeMatrixPath.getDataContainerName());
    if(nullptr == dc.get())
    {
      dc = dca->createNonPrereqDataContainer(this, m_CreatedEnsembleAttributeMatrixPath.getDataContainerName());
    }

    QVector<size_t> tDims(1,2); // we need 2 slots in the array. ZERO=Junk, 1 = our new primary stats data
    AttributeMatrix::Pointer cellEnsembleAttrMat = dc->createNonPrereqAttributeMatrix(this, m_CreatedEnsembleAttributeMatrixPath.getAttributeMatrixName(), tDims, SIMPL::AttributeMatrixType::CellEnsemble);
