...
void ReconstructionFunc::find_border()
{
  ...
  if(seed >= 0)
  {
	  ...
	  m_Grains[0].voxellist = new std::vector<int>(size);
	  m_Grains[0].voxellist->swap(voxelslist);
  }
}
  ...
  while (noseeds == 0)
  {
    ...
    if (seed >= 0)
    {
      ...
      if (size >= minallowedgrainsize)
      {
        ...
        {
       //   m_Grains.resize(graincount + 100, Grain());
        }
        voxelslist.clear();
        voxelslist.resize(initialVoxelsListSize, -1);
      }
    ...
    while (keepgoing > 0)
    {
      ...
      for (size_t j = 0; j < vlist->size(); j++)
      {
        ...
        if (voxels[index].kernelmisorientation <= 5.0 && voxels[index].alreadychecked == 0)
        {
          ...
          m_Grains[newnumgrains].nucleus = index;
          m_Grains[newnumgrains].active = 1;
          size++;
          ...
          voxelslist.resize(initialVoxelsListSize, -1);
          newnumgrains++;
          if (newnumgrains >= m_Grains.size()) {
            m_Grains.resize(newnumgrains + 100, Grain());
          }
        }
      }
    }
...
//  int index;
  ...
  for(int i=0;i<(xpoints*ypoints*zpoints);i++)
  {
	  if(voxels[i].grainname > 0 && voxels[i].unassigned != 1)
	  {
		...
		gnum = voxels[i].grainname;
		if(m_Grains[gnum].kernelmisorientation > voxels[i].kernelmisorientation)
		{
			m_Grains[gnum].nucleus = i;
...
void ReconstructionFunc::volume_stats2D(string writename1,string writename2,string writename3)
{
  double actualgrains = 0;
  double misocount = 0;
  double avgvol = 0;
...
//
// -----------------------------------------------------------------------------
int ReconstructionFunc::writeIPFVizFile(const std::string &file, double quat_symmhex[12][5])
{
  FILE* f = NULL;
  f = fopen(file.c_str(), "w");
  if (NULL == f) {return 1;}
  ...
  for (size_t i = 0; i < total; i++)
  {
    ...
    if(crystruct == 1)
    {
        calculateHexIPFColor(q1, quat_symmhex, red, green, blue);
    }
    fprintf(f, "%f %f %f\n",red, green, blue);
  }
...
//
// -----------------------------------------------------------------------------
void ReconstructionFunc::calculateHexIPFColor(double q1[4], double quat_symmhex[12][5],
                          double &red, double &green, double &blue)
{
  double qc[4];
  double p[3];
