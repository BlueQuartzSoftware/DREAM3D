...
#if 0
// -i C:\Users\GroebeMA\Desktop\NewFolder --outputDir C:\Users\GroebeMA\Desktop\NewFolder -f Slice_ --angMaxSlice 400 -s 1 -e 30 -z 0.25 -t -g 10 -c 0.1 -o 1.0 -x 1
#endif
...
//
// -----------------------------------------------------------------------------
void MicroGen3D::initialize(double stepX, double stepY, double stepZ,
                  ...
                  bool v_alreadyformed)
{
  ...
  diambin = new Bin[50];
  boverabin = new Bin[50];
  coverabin = new Bin[50];
  coverbbin = new Bin[50];
  seNbin = new Bin[1000];
...
void  MicroGen3D::find_boundarycenters(string writename13)
{
  ...
  for(int i = 0; i < numgrains; i++)
  {
    ...
    for(int k=0;k<size;k++)
    {
      ...
      bcent[count].x = bcent[count].x/tempcount;
      bcent[count].y = bcent[count].y/tempcount;
      bcent[count].z = bcent[count].z/tempcount;
      count++;
    }
  }
  ofstream outFile;
  ...
  {
    int grain1 = bcent[l].grain1;
    int grain2 = bcent[l].grain2;
    double xc = bcent[l].x;
    double yc = bcent[l].y;
    double zc = bcent[l].z;
    if(xc != 0 || yc != 0 || zc != 0)
