...
// function prototypes...
void initialize_micro (struct voxel *p, int ns, int xDim, int yDim, int zDim);
...
void get_nodes_fEdges (struct face *sq, struct voxel *p, struct neighbor *n, struct node *v, struct segment *e,
		       int eT2d[20][8], int nsT2d[20][8], int ns, int nsp, int xDim, int zID);
...
int  get_number_triangles (struct voxel *p, struct face *sq, struct node *v, struct segment *e, 
...
int  get_triangles (struct patch *t, struct face *sq, struct node *v, struct segment *e, 
		    int ns, int nsp, int xDim);
void get_case0_triangles (struct patch *t1, int *afe, struct node *v1, struct segment *e1, 
			  int nfedge, int tin, int *tout);
void get_case2_triangles (struct patch *t1, int *afe, struct node *v1, struct segment *e1, 
			  int nfedge, int *afc, int nfctr, int tin, int *tout);
void get_caseM_triangles (struct patch *t1, int *afe, struct node *v1, struct segment *e1, 
			  int nfedge, int *afc, int nfctr, int tin, int *tout, int ccn);
void arrange_spins (struct voxel *p, struct patch *t, struct node *v, struct neighbor *n, 
		    int numT, int xDim, int nsp, int zID);
...
void get_output_nodes (struct node *v, int nsp, int zID, int zDim);
void get_output_edges (struct segment *fe, struct isegment *ie, 
		       int nfe, int tnie, int zID, int zDim, int ceid, int *feid);
void get_output_triangles (struct patch *t, int nt, int zID, int ctid);
...
int main(void){
  ...
  initialize_micro(point, NS, xnum, ynum, znum);
  ...
  if( (f1=fopen("nodes_raw.txt", "w")) == NULL){
  ...
  for(i=0; i<(znum-1); i++){
    ...
    get_output_nodes (cVertex, NSP, i, znum);
    get_output_edges (cFedge, cIedge, nFEdge, tnIEdge, i, znum, cEdgeID, &fEdgeID);
    get_output_triangles (cTriangle, nTriangle, i, cTriID);
    cEdgeID = fEdgeID; 
  ...
  if( (f4=fopen("mesh_stat.txt", "w")) == NULL){
    printf("\nThe input file doesn't exist!\n");
    exit(1);
  }
...
void read_edge_neighspin_table(int eT2d[20][8], int nsT2d[20][8]){
  ...
  if( (f1=fopen("edgeTable_2d.txt", "r")) == NULL){
    printf("\nThe input file doesn't exist!\n");
    exit(1);
  }
 
  if( (f2=fopen("neighspinTable_2d.txt", "r")) == NULL){
    printf("\nThe input file doesn't exist!\n");
    exit(1);
  }
...
int get_number_fEdges (struct face *sq, struct voxel *p, struct neighbor *n, int nsp, int zID){
  ...
  for(k=0; k<(3*2*nsp); k++){       // for each square on upper layer...
	...
	sq[k].effect = 1;   // mark current square as effective one that can be "marching cubed"...
      ...
      if(cspin>0 && atBulk==0){  // consider the square inside the bulk only...
...
void get_nodes_fEdges (struct face *sq, struct voxel *p, struct neighbor *n, struct node *v, struct segment *e,
  ...
  for(k=0; k<3*2*nsp; k++){       // for each square...
...
int get_square_index(int tns[4]){
  ...
  if(tns[0]==tns[1]){aBit[0] = 0;}else{aBit[0] = 1;}
  if(tns[1]==tns[2]){aBit[1] = 0;}else{aBit[1] = 1;}
  if(tns[2]==tns[3]){aBit[2] = 0;}else{aBit[2] = 1;}
  if(tns[3]==tns[0]){aBit[3] = 0;}else{aBit[3] = 1;}
  if(tns[0]==tns[2]){aBit[4] = 0;}else{aBit[4] = 1;}
  if(tns[1]==tns[3]){aBit[5] = 0;}else{aBit[5] = 1;}
...
int treat_anomaly(int tnst[4], struct voxel *p1, struct neighbor *n1, int nsp1, int zID1){
  ...
  if(minid==1 || minid==3){  // we can reverse this...let's try this later...
    tempFlag = 0;
  }else{
    tempFlag = 1;
  }
...
void get_nodes(int cst, int ord, int nidx[2], int *nid, int nsp1, int xDim1){
  ...
  for(ii=0; ii<2; ii++){
    ...
    if(ord==0){         // if the square index is 0 for corner site...
    ...
    }else if(ord==1){   // if the square index is 1...
...
void get_spins(int nSpn[4], int pID[2], int *pSpin){
  ...
  for(i=0; i<2; i++){
    ...
    switch(pixTemp){	
    case 0:  tempSpin = nSpn[0];   pSpin[i] = tempSpin;  break;
    case 1:  tempSpin = nSpn[1];   pSpin[i] = tempSpin;  break;
    case 2:  tempSpin = nSpn[2];   pSpin[i] = tempSpin;  break;
    case 3:  tempSpin = nSpn[3];   pSpin[i] = tempSpin;  break;	
    }
...
int  get_number_triangles (struct voxel *p, struct face *sq, struct node *v, struct segment *e, 
  ...
  for(i=1; i<=nsp; i++){
	...
	printf("Something wrong with counting number of edges for marching cube...\n");
      ...
      if(nFC==0){  // when there's no face center 
...
int  get_number_case2_triangles (int *afe, struct node *v1, struct segment *e1, 
				 int nfedge, int *afc, int nfctr){

  int ii, i, j, jj, k, kk, k1, m, n, i1, j1;
  ...
  for(j1=1; j1<loopID; j1++){
    ...
    if(openL==1){  // if current loop is an open loop...
    ...
    }else{   // if current loop is a closed one....i.e., openL = 0...
...
int  get_number_caseM_triangles (int *afe, struct node *v1, struct segment *e1, 
				 int nfedge, int *afc, int nfctr){

  int ii, i, j, jj, k, kk, k1, m, n, i1, j1, n1;
...
void get_caseM_triangles (struct patch *t1, int *afe, struct node *v1, struct segment *e1, 
  ...
  int ii, i, j, jj, k, kk, k1, m, n, i1, j1, n1, iii;
...
void arrange_spins (struct voxel *p, struct patch *t, struct node *v, struct neighbor *n, 
  ...
  for(i=0; i<numT; i++){    // for each triangle...
    ...
    for(j=0; j<3; j++){    // for each node inside the triangle...
...
int get_inner_edges (struct node *v, struct segment *fe, struct isegment *ie, struct patch *t, int nfe, int nT){
  ...
  for(i=0; i<nT; i++){   // for each triangle...
    
    for(j=0; j<3; j++){  // for each edge of the current triangle...
	...
	index2 = 0;
      ...
      if((cnk1>10) || (cnk2>10)){  // edges connected to body centers...
	t[i].edgePlace[j] = 1;
...
void find_unique_inner_edges (struct isegment *ie, int nie, int *nEff){
  ...
  for(i=0; i<nie; i++){
	...
	if(nbflag==-1){
	  ...
	  if( (cn1==nn1 && cn2==nn2) || (cn1==nn2 && cn2==nn1)){  // if they are the same nodes...
	    ...
	    for(k=0; k<numCN; k++){   // remember nns1 is different from nns2...
...
void get_output_nodes (struct node *v, int nsp, int zID, int zDim){
  ...
  if( (f=fopen("nodes_raw.txt", "a+")) == NULL){
    printf("\nThe input file doesn't exist!\n");
    exit(1);
  }
...
void get_output_edges (struct segment *fe, struct isegment *ie, 
  ...
  char postfix[64] = "_edges_raw.txt";
  char buf[BUFSIZ];
  sprintf(buf, "%d%s", zID, postfix);
