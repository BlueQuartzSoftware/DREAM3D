...
//
// -----------------------------------------------------------------------------
RotationTransforms::FatalError(char* func, char* msg)
{
  std::cout << std::string(func) << "::" << std::string(msg) << std::endl;
}
...
res = 1;
if ((eu(1) < 0.0) || (eu(1) > (2.0*M_PI))) {
 FatalError('rotations:eu_check','phi1 Euler angle outside of valid range [0,2pi]');
  }
if ((eu(2) < 0.0) || (eu(2) > M_PI)) {
 FatalError('rotations:eu_check','Phi  Euler angle outside of valid range [0,pi]');
  }
if ((eu(3) < 0.0) || (eu(3) > (2.0*M_PI))) {
 FatalError('rotations:eu_check','phi2 Euler angle outside of valid range [0,2pi]');
  }
res = 0;
}
...
res = 1;
if ((eu(1) < 0.0) || (eu(1) > (2.0*M_PI))) {
 FatalError('rotations:eu_check_d','phi1 Euler angle outside of valid range [0,2pi]');
  }
if ((eu(2) < 0.0) || (eu(2) > cPi)) {
 FatalError('rotations:eu_check_d','Phi  Euler angle outside of valid range [0,pi]');
  }
if ((eu(3) < 0.0) || (eu(3) > (2.0*M_PI))) {
 FatalError('rotations:eu_check_d','phi2 Euler angle outside of valid range [0,2pi]');
  }
res = 0;
}
...
res = 1;
if (ro(4) < 0.0) {
 FatalError('rotations:ro_check','Rodrigues-Frank vector has negative length');
  }
if (abs(sqrt(sum(ro(1:3)*ro(1:3)))-1.0) > eps) {
 FatalError('rotations:ro_check','Rodrigues-Frank axis vector not normalized');
  }
res = 0;
}
...
res = 1;
if (ro(4) < 0.0) {
 FatalError('rotations:ro_check_d','Rodrigues-Frank vector has negative length');
  }
if (abs(sqrt(sum(ro(1:3)*ro(1:3)))-1.0) > eps) {
 FatalError('rotations:ro_check_d','Rodrigues-Frank axis vector not normalized');
  }
res = 0;
}
...
r = sqrt(sum(ho*ho))
if (r > LPs%R1) {
 FatalError('rotations:ho_check_d','homochoric vector outside homochoric ball');
  }
res = 0;
}
...
do i=1,3;
  do j=1,3;
if (dabs(res(i,j)) < thr) res(i,j) = 0.0
  end do;
end do;
}
...
if (res(1) < 0.0) res(1) = mod(res(1)+100.0*cPi,2.0*cPi)
if (res(2) < 0.0) res(2) = mod(res(2)+100.0*cPi,cPi)
if (res(3) < 0.0) res(3) = mod(res(3)+100.0*cPi,2.0*cPi)
}
