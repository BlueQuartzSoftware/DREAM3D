...
namespace OrientationPrinters
{
  ...
  // -----------------------------------------------------------------------------
  template<typename T, typename K>
  void Print_QU(const T& om, typename QuaternionMath<K>::Order layout = QuaternionMath<K>::QuaternionVectorScalar)
  {
  ...
  // -----------------------------------------------------------------------------
  template<typename K>
  void PrintTuple(DataContainerArray::Pointer dca, size_t t)
  {
    for(int i = 0; i < 7; i++)
    {
  ...
  // -----------------------------------------------------------------------------
  template<typename K>
  void PrintTuple(typename DataArray<K>::Pointer data, size_t t)
  {
    ...
    for (int a = 0; a < cDims[0]; a++)
    {
      ...
      if (a < cDims[0] - 1) { printf("  "); }
      if (cDims[0] == 9 && (a == 2 || a == 5) ) { printf("\n"); }
    }
    printf("\n");
  }
