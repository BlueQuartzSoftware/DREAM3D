...
void FindBasalLoadingFactor::dataCheck(bool preflight, size_t voxels, size_t features, size_t ensembles)
{
  ...
  VolumeDataContainer* m = getDataContainerArray()->getPrereqDataContainer<VolumeDataContainer, FindBasalLoadingFactor>(getDataContainerName(), false, this);
  ...
  dims[0] = 1;
  m_BasalLoadingFactorPtr = m->createNonPrereqArray<DataArray<float>, AbstractFilter, float>(this, m_CellFeatureAttributeMatrixName,  m_BasalLoadingFactorArrayName, 0, features, dims); /* Assigns the shared_ptr<> to an instance variable that is a weak_ptr<> */
